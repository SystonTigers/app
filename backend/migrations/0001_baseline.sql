-- 0001_baseline.sql
-- Production schema as of 2026-09-23 (exported from D1 syston-db).
-- Every statement is IF NOT EXISTS, so this is a no-op on production and
-- builds the same starting point for local, test and new environments.
...
CREATE INDEX IF NOT EXISTS idx_fixtures_tenant_date ON "fixtures_old"(tenant_id, fixture_date, kick_off_time);

CREATE INDEX IF NOT EXISTS idx_fixtures_tenant_status ON "fixtures_old"(tenant_id, status);
